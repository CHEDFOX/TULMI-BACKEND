<!--
  ...
  Placeholders filled by the backend:
    ...
    {{RECIPIENT_HINT}} → per-contact hint the user saved (or "")
    {{TONE_DIAL}}      → three named dials (0-100), or "Default." if none set
    {{APP_STYLE}}      → per-app override block, or ""
    {{WATERMARK}}      → "on" | "off"

  v2 adds: tone dial, per-app style, per-recipient hint, watermark.
-->
...
- **formality** raises/lowers register (0 slangy · 100 formal).
- **length** shortens/expands the reply relative to a natural length for the intent.
- **warmth** shifts from matter-of-fact toward personable/kind.
...
If "on", append " · Tailzu" at the very end of the reply on a single output
line only — never inside an email sign-off block, never in a code fence, and
skip it if the total reply is under 20 characters.
